...
Данное веб-приложение будет иметь 3 стандартных уровня архитектуры: Frontend, Backend, Database. Вполне достаточный набор уровней для веб-приложения по учету заказов и просмотра меню, так как весь функционал приложения по сути строится на взаимодействии с базой данных, поэтому больше уровней не требуется(разве что кэширования некоторых данных, но я думаю что это будет нужно уже при масштабировании системы, пока можно и без этого).

## 2. Основные компоненты приложения

### Пользовательский интерфейс

С помощью него пользователи взаимодействуют с приложением

### REST API

Протокол обмена информацией между frontend'ом и backend'ом приложения с помощью (думаю GET и POST запросов будет вполне достаточно, возможно можно будет еще реализовать DELETE)

### Бизнес-логика приложения

Логика создания заказов, задания им статуса, контроля меню, взаимодействия с заказами, оплата заказов

### Сервис уведомлений

Необходим для своевременного информирования пользователя о статусе заказа, администратора о каких-то ситуациях и т. п. вещей

### Модуль работы с данными

Сериализаторы, функции взаимодействия с базами данных, в общем все, что нужно для передачи и/или хранения информации

### База данных (postgres)

Обеспечивает хранение данных о заказах и пользователях

### Система аутентификации

Служит для авторизации пользователей в системе, разделяя права пользователей под разными типами аккаунтов (клиент/админ/работник)

## 3. Взаимодействие компонентов

![Диаграмма взаимодействия](images/sequence-diagram.png)

## 4. Определение индивидуальной особенности предметной области

Уведомления пользователям о статусе их заказов и уведомления для работников и администратора будут осуществляться с помощью сервиса уведомлений, он уже отражен на диаграмме архитектуры
